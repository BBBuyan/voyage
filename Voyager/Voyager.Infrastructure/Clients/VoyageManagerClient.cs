...
public class VoyageManagerClient : IVoyageManagerClient
{
    ...
    public async Task<VoyagerAgentCredentials> SendEnrollRequestAsync(
    ...
    public async Task<AgentToken> FetchTokenAsync(
        VoyagerAgentCredentials creds,
        CancellationToken ct
    )
    {
        TokenRequest request = new() { AgentId = creds.AgentId, Password = creds.Password };
        using HttpResponseMessage response = await _httpClient.PostAsJsonAsync(
            "token",
            request,
            ct
    ...
    public async Task<AgentCommand?> CheckInAsync(string token, CancellationToken ct)
    {
        ...
        CheckInResponse? checkInResponse =
            await response.Content.ReadFromJsonAsync<CheckInResponse>(JsonOptions, ct);
    ...
    public async Task SendCommandStatus(
        string token,
    ...
    )
    {
        ...
        using HttpRequestMessage requestMessage = new(
            HttpMethod.Post,
            $"commands/{agentCommand.Id}/status"
        );
        requestMessage.Headers.Authorization = new("Bearer", token);
        ...
        CommandStatusResponse? commandStatusResponse =
            await response.Content.ReadFromJsonAsync<CommandStatusResponse>(JsonOptions, ct);
    }
}
